    ...
    Public Sub Update(ByVal rowCount As Long)
        If gvMain IsNot Nothing Then
            If gvMain.PageCount > 0 Then
                ...
                lblSummary.Text = "Row " & ((gvMain.PageIndex * gvMain.PageSize) + 1).ToString(Constant.IntFormat) & " - " & ((gvMain.PageIndex * gvMain.PageSize) + gvMain.Rows.Count).ToString(Constant.IntFormat) & " From " & rowCount.ToString(Constant.IntFormat) & " Row(s)"
                txtPageIndex.Text = "0"
            Else
                Me.Visible = False
            End If
